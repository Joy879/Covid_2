...
server <- function(input,output) {
  ...
  output$wmap <- renderPlotly ({
    ...
    p<-mapdata%>% ggplot(aes(x = long,
      ...
      coord_fixed(ratio=1, xlim = c(-100, 100), ylim = c(-100, 100), clip="off")+ 
  ...
  regiongraph <- function(region, years, cases, color){
    data <- read_csv(glue('data/covid{years}.csv'))
    graph<-data%>%filter(WHO_region == region)%>%ggplot(aes(y=get(cases),
                                                            x=Date_reported,
                                                            text=paste(cases, get(cases), "<br>")))+
      geom_col(fill=color)+
      labs(y=cases)+
